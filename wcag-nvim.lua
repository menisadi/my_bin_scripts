...
local THRESHOLD = {
  aa  = { content = 4.5, secondary = 4.5, ui = 3.0, other = 4.5 },
  aaa = { content = 7.0, secondary = 4.5, ui = 3.0, other = 7.0 },
}
local APCA_TARGET = { content = 60, secondary = 45, ui = 30, other = 60 }
local TIER_LABEL = {
...
local TIER_ORDER = { "content", "secondary", "ui", "other" }
...
local UI_CHECKS = {
  ...
  { "MatchParen", "bg", "Normal", "bg" },
  { "Cursor", "bg", "Normal", "bg" },
  { "Pmenu", "bg", "Normal", "bg" },
  { "PmenuSel", "bg", "Pmenu", "bg" },
  { "PmenuThumb", "bg", "PmenuSbar", "bg" },
  { "NormalFloat", "bg", "Normal", "bg" },
  { "FloatBorder", "fg", "NormalFloat", "bg" },
  { "WinSeparator", "fg", "Normal", "bg" },
  { "VertSplit", "fg", "Normal", "bg" },
  { "StatusLine", "bg", "Normal", "bg" },
  { "TabLineSel", "bg", "TabLineFill", "bg" },
  { "DiffAdd", "bg", "Normal", "bg" },
  { "DiffDelete", "bg", "Normal", "bg" },
}
...
local function record(tier, fg, bg, layer, group)
  local t = THRESHOLD[opts.level][tier]
  local key = table.concat({ tier, hex(fg), hex(bg), layer }, "|")
  local c = counted[key]
  if c then
...
for _, u in ipairs(UI_CHECKS) do
  ...
  if ca and cb and hex(ca) ~= hex(cb) then
    record("ui", ca, cb, u[3], u[1] .. " vs " .. u[3])
  end
end
...
io.write(dim("  (pass rate on the buffer background / over all layers)\n"))
io.write(("  %s  content %s  ·  secondary %s  ·  ui %s"):format(
  bold("Score"), score(stats.content), score(stats.secondary), score(stats.ui)))
